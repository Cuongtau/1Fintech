...
function ConfirmCodeBankDirect(orderId, sign) {
    ...
    utils.postData(utils.portalUrl() + "PaymentByBankConfirm", param, function (data) {
        ...
        if (data.c >= 0) {
            utils.showPopupConfirm("", i18n.t('homepage.IKnew'), 1, url);
            if (url != "") {
                setTimeout(function () {
                    window.location = url;
                },5000);
            ...
            utils.showPopupConfirm(data.m, i18n.t('homepage.Retry'), 2, url);
    },
        function (data) {
            console.log(data);
            debugger;
            ...
            utils.unLoading();
            var url = (data.d.urlRedirect == undefined || data.d.urlRedirect == null || data.d.urlRedirect == "") ? '' : 'window.location="' + data.d.urlRedirect + '"';
            var msg = "";
            switch (data.c) {
                case -10145://số tiền thanh toán quá nhỏ
                    msg = i18n.t('payment.minAmountInvalid');
            ...
            if (url != '') {
                utils.showPopupConfirm(msg, i18n.t('homepage.Retry'), 2, url);
                setTimeout(function () {
                    window.location = url;
                }, 5000);
            }
